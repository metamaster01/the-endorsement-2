...
//             Our Services
//           </motion.div>
//           <motion.h2 className="text-4xl font-bold text-white mb-6 font-poppins">
//             Complete <span className="text-merkurie-accent">Celebrity Marketing</span> Solutions
//           </motion.h2>
//           <motion.p className="text-xl text-white/70 max-w-3xl mx-auto">
        ...
        >
          {services.map((service, index) => (
            <motion.div key={index} custom={index}>
              <GlowCard
                customSize={true}
                className="w-full h-auto min-h-[300px] bg-merkurie-surface/50 backdrop-blur-sm border border-white/10 md:hover:border-merkurie-coral/30 transition-all duration-300 md:hover:transform md:hover:scale-105 p-8"
                glowColor={index % 3 === 0 ? "merkurie-accent" : index % 3 === 1 ? "merkurie-coral" : "merkurie-teal"}
              >
                <div className="flex flex-col h-full">
                  <motion.div
                    className="w-12 h-12 bg-merkurie-coral/20 rounded-lg flex items-center justify-center mb-6 md:group-hover:bg-merkurie-coral/30 transition-all duration-300"
                    whileHover={
